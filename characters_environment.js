...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press 'Play' button to start & play ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
...
function start_game() //Initializing Game
{
  game_status = "Starting"; //Should be in sync with the nose X and Y - otherwise the console values will appear as soon as the webpages open and the game will not start (Flag Variables)
...
function changeGameStatud(character) {
  if (noseX != "" && game_status == "Starting" && gameConfig.status === "start") {
    world_start.play();
